...
2. Prepare a reviewable local draft, identify the exact provider target and create/update scope, and show the intended change. Reuse explicit authorization already given for this content and target; if it is missing, obtain it immediately before the provider write. Preparing the draft does not require a separate confirmation.
...
4. Before the provider write, complete the following checks using the authorization established in step 2:
...
5. Finalize the local knowledge-base note with required frontmatter (`graduated_from` and non-empty `graduated_by` mandatory; see `frontmatter.md`), without yet marking the project topic as successfully graduated. When choosing `tags`, check the provider's existing tag inventory first (Obsidian: `obsidian tags counts`) and prefer reusing an existing tag over coining a near-synonym — consumption-time tag-query recall depends on this discipline (see `provider-interface.md` → Read side).
6. Execute the selected provider write only for the authorized target and scope after its required preflight succeeds. Read back the result; distinguish a confirmed write with failed verification from a fully verified graduation, and do not repeat a create merely because verification failed. After a verified write, record `graduated_by` on the project topic and optionally `graduated_to` / `graduated_at`; if verification is incomplete, record the actual write/verification state in the log without claiming completion.
7. Record the graduation itself as one `cairn/LOG.md` entry — summary plus a pointer to the knowledge-base note — the same way an audit run logs itself (see `audit.md` → Behavior).
